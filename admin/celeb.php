...
<head>
	...
	<title>Celebrities | Doctor T </title>
...
<body class="animsition dashboard">
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = 'celeb_add.php';">
							                      	<i class="icon wb-plus" aria-hidden="true"></i> Add Reviews
							                    </button>
			                  				</div>
			                  			</div>
			                  		</div>
			                  				...
			                  				<!-- <table id="exampleTablePagination" data-toggle="table" data-url="../assets/data/bootstrap_table_test.json" data-query-params="queryParams" data-mobile-responsive="true" data-height="400" data-pagination="true" data-icon-size="outline" data-search="true" class="table table-hover" style="margin-top: -36px;"> -->
			                    				<thead style="">
			                    					<tr>
			                    						...
			                    						<th style="" data-field="recommend" data-align="center" data-width="120px">
			                    							<div class="th-inner ">Select Show</div>
			                    							<div class="fht-cell"></div>
			                    						</th>
			                    						<th style="" data-field="">
			                    				...
			                    				<?php if(!empty($celeds_list)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($celeds_list as $celeds_detail) : 
													?>
			                  						<tr> 
			                  							<td class=""><?php echo $i+1; ?></td> 
														<td style=""><?php echo $celeds_detail->name; ?></td>
														<td style="">
															<?php if ($celeds_detail->is_selected == '1') : ?>
																<button class="btn btn-sm btn-success btn-icon btn-floating" type="button" onclick="clChange(<?php echo $celeds_detail->id; ?>, 0)"><i class="icon wb-check" aria-hidden="true"></i></button>
															<?php else : ?>
																<button class="btn btn-sm btn-danger btn-icon btn-floating" type="button" onclick="clChange(<?php echo $celeds_detail->id; ?>, 1)"><i class="icon wb-close" aria-hidden="true"></i></button>
															<?php endif; ?>
														</td>
			                  							<td style=""><?php echo date("d/m/Y", strtotime( $celeds_detail->create_date ) ); ?></td>
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = 'celeb_edit.php?id=<?php echo $celeds_detail->id; ?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delP(<?php echo $celeds_detail->id; ?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<input type="hidden" id="celeb_id" name="celeb_id">
					<input type="hidden" id="clStatus" name="clStatus">
					<button type="button" class="btn btn-success" onclick="changeCl()"><i class="fa fa-check"></i> Confirm</button>
    ...
    function changeCl()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'funcQuery.php',
		  	data 	: {cid:$('#celeb_id').val(), st:$('#clStatus').val(), action:'changeCeleb'},
	...
	function delP(celeb_id)
    {
    	$('#celeb_id').val(celeb_id);
     ...
     function deleteProduct()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'funcQuery.php',
		  	data 	: {celeb_id:$('#celeb_id').val(), action:'delceleds'},
		  	success: function(data) {
		        	if (data == 'true') {
		        		location.reload();
		        	} else {
