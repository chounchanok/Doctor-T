...
<body class="animsition dashboard">
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = 'promotion_add.php';">
							                      	<i class="icon wb-plus" aria-hidden="true"></i> Add Promotion
							                    </button>
			                  				</div>
			                  			</div>
			                  		</div>
			                  				...
			                  				<!-- <table id="exampleTablePagination" data-toggle="table" data-url="../assets/data/bootstrap_table_test.json" data-query-params="queryParams" data-mobile-responsive="true" data-height="400" data-pagination="true" data-icon-size="outline" data-search="true" class="table table-hover" style="margin-top: -36px;"> -->
			                    				...
			                    				<?php if(!empty($promo_list)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($promo_list as $promo_detail) : 
													?>
			                  						<tr> 
			                  							<td class=""><?php echo $i+1; ?></td> 
			                  							<td style=""><?php echo $promo_detail->title; ?></td>
														<td style="">
															<?php if ($promo_detail->is_selected == '1') : ?>
																<button class="btn btn-sm btn-success btn-icon btn-floating" type="button" onclick="promoChange(<?php echo $promo_detail->id; ?>, 0)"><i class="icon wb-check" aria-hidden="true"></i></button>
															<?php else : ?>
																<button class="btn btn-sm btn-danger btn-icon btn-floating" type="button" onclick="promoChange(<?php echo $promo_detail->id; ?>, 1)"><i class="icon wb-close" aria-hidden="true"></i></button>
															<?php endif; ?>
														</td>
			                  							<td style=""><?php echo date("d/m/Y", strtotime( $promo_detail->create_date ) ); ?></td>
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = 'promotion_edit.php?id=<?php echo $promo_detail->id; ?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delS(<?php echo $promo_detail->id; ?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
	...
	function promoChange(pmid, status)
    {
    	$('#promo_id').val(pmid);
    	$('#pStatus').val(status)
    ...
    function changeSt()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'funcQuery.php',
		  	data 	: {pmid:$('#promo_id').val(), st:$('#pStatus').val(), action:'changePromo'},
     ...
     function deleteSlide()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'funcQuery.php',
		  	data 	: {promotion_id:$('#promotion_id').val(), action:'delpromotion'},
		  	success: function(data) {
		        	if (data == 'true') {
		        		location.reload();
		        	} else {
