...
<body class="animsition dashboard">
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = 'slide_add.php';">
							                      	<i class="icon wb-plus" aria-hidden="true"></i> Add Slide
							                    </button>
			                  				</div>
			                  			</div>
			                  		</div>
			                  				...
			                  				<!-- <table id="exampleTablePagination" data-toggle="table" data-url="../assets/data/bootstrap_table_test.json" data-query-params="queryParams" data-mobile-responsive="true" data-height="400" data-pagination="true" data-icon-size="outline" data-search="true" class="table table-hover" style="margin-top: -36px;"> -->
			                    				...
			                    				<?php if(!empty($slide_list)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($slide_list as $slide_detail) : 
													?>
			                  						<tr> 
			                  							<td class=""><?php echo $i+1; ?></td> 
			                  							<td style=""><?php echo $slide_detail->name; ?></td>
			                  							<td style=""><?php echo date("d/m/Y H:i:s", strtotime( $slide_detail->create_date ) ); ?></td>
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = 'slide_edit.php?id=<?php echo $slide_detail->id; ?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delS(<?php echo $slide_detail->id; ?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
     ...
     function deleteSlide()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'funcQuery.php',
		  	data 	: {sid:$('#s_id').val(), action:'delSlide'},
		  	success: function(data) {
		        	if (data == 'true') {
		        		location.reload();
		        	} else {
